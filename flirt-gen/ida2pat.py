...
def zrange(*args):
    ...
    if len(args) == 1:
        ...
        raise RuntimeError("Invalid arguments provided to zrange: {:s}".format(
            str(args)))
    if end < start:
        raise RuntimeError(
            "zrange only iterates from smaller to bigger numbers only: {:d}, {:d}"
            .format(start, end))
...
CRC16_TABLE = [
    0x0, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48,
...
def make_func_sig(config, func):
    ...
    if loc < func.endEA - func.startEA:
        tail = " "
        for ea in zrange(func.startEA + loc,
                         min(func.endEA, func.startEA + 0x8000)):
            if ea in variable_bytes:
                tail += ".."
            else:
...
def make_func_sigs(config):
    ...
    if config.mode == USER_SELECT_FUNCTION:
        ...
        for i, f in enumerate(get_functions()):
            try:
                logger.info("[ %d / %d ] %s %s", i + 1, n, get_name(f.startEA),
                            hex(f.startEA))
                sigs.append(make_func_sig(config, f))
            except FuncTooShortException:
                pass
...
def get_pat_file():
    ...
    filename = ida_kernwin.ask_file(1, name, "Enter the name of the pattern file")
    if filename is None:
