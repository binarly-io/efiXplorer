...
public:
  ...
  std::map<json, std::string>
      dbProtocolsMap; // a map to look up a GUID name by value
  json bootServices;
  json peiServices;
  json peiServicesAll;
  json ppiCallsAll;
  ...
  string_list_t ppiPEINames = {"InstallPpi", "ReInstallPpi", "LocatePpi",
                               "NotifyPpi"};
};
